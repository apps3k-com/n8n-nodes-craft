...
- **Node & credential help links** now resolve. The codex `node` field still referenced the upstream package name (`n8n-nodes-craft.*`), so n8n never bound the documentation metadata to these nodes and the in-editor help links pointed nowhere. Corrected to `n8n-nodes-craft-apps3k.*`, and pointed `primaryDocumentation`, `credentialDocumentation`, and both credentials' `documentationUrl` at the user guide ([craft-n8n.apps3k.com](https://craft-n8n.apps3k.com/)).

## [2.3.0] - 2026-05-30

### Added

- **File upload — File Name** option. The upload operation accepts an optional **File Name** (defaults to the uploaded file's original name) and returns it in the node output as `fileName`. Craft's API does not store file names on uploaded blocks, so this surfaces the name for downstream nodes only.

## [2.2.1] - 2026-05-30

### Fixed

- **File upload** now reliably targets the `/upload` endpoint. The File resource's single (default) operation was not persisted by n8n, so the operation-level route was skipped and the request went to the API base URL — which 307-redirected to an HTML handler and returned `500 "Only HTML requests are supported here"`. The upload route + binary `preSend` now live on the **File resource** itself, so they apply whether or not the operation is explicitly selected. (The Block → Upload File path was unaffected.)

## [2.2.0] - 2026-05-30

### Added

- **File upload** — upload images, videos, and documents into Craft via a new **File → Upload** operation (also available as **Block → Upload File**), in both nodes. Provide the file as binary input; choose position and target (page/date/sibling). Returns `blockId` and `assetUrl`.
- **User guide** — the README is now a usage guide covering setup, both nodes, collection field mapping, and uploads.

## [2.1.0] - 2026-05-30

### Added

- **Schema-driven field mapping for collection items** — *Add Items* and *Update Items* now use a typed Resource Mapper instead of raw JSON. Pick a collection and its fields render as native inputs (text, number, date picker, and select dropdowns populated from the collection's schema), in both the Craft Documents and Craft Daily Notes nodes.
- **Relation picker** — link items to related collections via a multi-select that lists the target collection's items by name (resolved from each relation's `targetCollectionId`).

### Changed

- Collection *Add/Update Items* now map one collection item per input item (n8n standard for data nodes); feed multiple input items to write multiple rows.

### Fixed

- *Update Items* now fails fast with a clear error when an Item ID is missing, instead of an opaque API rejection.
